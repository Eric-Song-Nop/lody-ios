import { NativeModule, requireNativeModule } from 'expo';

declare class DataRuntimeModule extends NativeModule<{
  onRecoveryPhase: (event: { phase: string }) => void;
}> {
  runRuntimeRecoveryVerification(): Promise<string>;
  runDataRuntimeVerification(): Promise<string>;
}

const native = requireNativeModule<DataRuntimeModule>('LodyKit');
export const runDataRuntimeVerification = () =>
  native.runDataRuntimeVerification();

export const runRuntimeRecoveryVerification = () =>
  native.runRuntimeRecoveryVerification();
export const addRecoveryPhaseListener = (
  listener: (event: { phase: string }) => void,
) => native.addListener('onRecoveryPhase', listener);
