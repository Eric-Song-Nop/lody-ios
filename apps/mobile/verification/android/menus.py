...
def run(shell, wait_text, tap_button, capture, texts, result, tree, appearance):
    ...
    for host in ('page', 'sheet'):
        ...
        tap_button(tree, 'Choose item filter')
        tree = wait_text('Recent items')
        if not any(node.get('checked') == 'true' for node in tree.iter('node')):
            raise AssertionError('Native menu has no selected item')
        ...
        wait_text('Selected: recent; selections: 1')
        ...
        tap_button(tree, 'Choose item filter')
        tap_button(wait_text('All items'), 'All items')
        wait_text('Selected: all; selections: 2')
