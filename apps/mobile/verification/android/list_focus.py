...
def run(shell, wait_text, tap_button, capture, texts, result, tree, appearance, list_return='present'):
    ...
    for host in ('page', 'sheet'):
        ...
        label = 'Disable source navigation' if list_return == 'disabled' else 'Remove source row'
        # Ordinary return must also prove that focus actually entered detail.
        # Otherwise focus retained under the sheet can falsely pass return.
        observations = []
