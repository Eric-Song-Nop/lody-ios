...
- This is an independent iOS and Android client, using a pnpm workspace, Expo Router, React Native, and one local Expo module. Android is being implemented through docs/plans/2026-09-10-android-pr-roadmap.md; do not claim unverified capabilities are available.
- All first-party native capabilities and native UI live in `apps/mobile/modules/lody-kit`. Register them through `LodyKitModule`; do not create separate bridge packages. Use Swift/UIKit or SwiftUI on iOS and Kotlin native capabilities on Android. Keep platform-specific adapters behind the typed kit facade.
- Implement Android in the staged PR order. Never add fallback stubs or fake-success native APIs. Unimplemented capabilities must stay outside the Android entry graph until their real implementation is available.
- `apps/mobile/ios` and `apps/mobile/android` are generated by Expo prebuild. Persist changes in app config, local config plugins when needed, and LodyKit, never only in generated files.
...
- Credentials belong in Keychain on iOS and Keystore-backed encrypted private storage on Android, through LodyKit. Never copy desktop credentials, tokens, transcripts, or private service configuration into this repository.
...
- On iOS, native stack headers use transparent headers and soft scroll edges; scrolling screens use `ScrollViewMarker` with the shared `softScrollEdgeEffects` and automatic content insets.
...
- Apple HIG is the primary iOS UI requirement. Android follows native Material conventions, uses at least 48 dp touch targets, and preserves TalkBack and system text behavior. The Expo chatbot reference informs chat behavior only; do not copy its branding over native conventions.
- Do not use green accents or green-tinted backgrounds. Use UIKit semantic colors on iOS and appropriate Material/system colors on Android, blue for actions, and neutral system backgrounds. Preserve automatic dark mode and accessibility contrast adaptation.
...
- Import native APIs only from `@lody-ios/kit`. Keep the typed NativeModule facade, platform View wrappers, and Swift/Kotlin implementations in matching feature directories. Module event subscriptions must be removed on unmount; UIKit work runs on the main queue; Android View/WebView work runs on the main thread.
...
- Swift on iOS and Kotlin on Android own the offscreen WebView and watchdog. Android must run the existing bundled JS/WASM data core; JavaScriptSandbox adoption requires the roadmap compatibility checks. Keep startup/heartbeat deadlines outside JS, ignore callbacks from replaced views, and bound automatic restarts. Background suspension is not a watchdog failure: retain the WebView and reset its heartbeat deadline on foreground; recreate only a missing or failed runtime. iOS 26 continued processing is tied to a user-started send through reply synchronization, never idle connection keepalive.
...
- Follow `docs/architecture/android-platform-contracts.md` for capability ownership and `docs/plans/2026-09-10-android-pr-roadmap.md` for PR dependencies.
- UIKit/CoreText/SF Symbols, Swift composer, AXe and iOS notification-extension rules above apply to iOS. Preserve their behavior while introducing corresponding Android implementations.
- Keep Android data and Diff WebViews under separate lifecycle owners. Only one live data replica owner exists for an account/workspace context.
- Each completed stage requires local Android emulator evidence. Emulator evidence does not establish real-device refresh-rate, haptic, push-provider or OEM background behavior.
- Push stacked branches only to the user fork; child PRs target their immediate parent branch. Do not merge or publish a release merely because a PR is created.
