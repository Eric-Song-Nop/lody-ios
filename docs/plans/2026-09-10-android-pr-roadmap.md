...
执行规模为 **6 个阶段、28 个必需 PR、3 个条件优化 PR**。直接开工时，从[逐 PR 文档的工作导航](2026-09-10-android-pr-checklist.md#使用方式)进入对应实施步骤；阶段数量、子 PR 数量和条件触发项分别统计，避免将 00～17 的能力包编号误当作实际 PR 数量。

每个审查单元的落地顺序见[28 个 PR 内的提交顺序](2026-09-10-android-pr-checklist.md#每个实际-pr-内的提交顺序)：从契约与固定输入，到真实实现，再到故障验证和下游交接。该表细化现有范围，不增加 PR 数量或改变完成状态。

评审范围按[每个 PR 在哪里收尾](2026-09-10-android-pr-checklist.md#每个-pr-在哪里收尾)逐项核对：明确本次必须闭合的行为、后续承接方，以及跨阶段故障的修复归属。Enriched 原型、流式正确性与生产定案分别收尾；完整 WASM 基线不与可选宿主优化混为一项。

执行时配合[逐 PR 交付清单](2026-09-10-android-pr-checklist.md)：该文按 28 个实际必需 PR 列出准入条件、代码落点、交接材料和阻塞条件，并展开 WASM、渲染选型、输入草稿、授权读取、可靠发送、Diff、文件、通知后台及发行的具体实施顺序；补充各阶段的连续验收流程、失败阻塞范围以及 PR-06 前需冻结的测试输入。实际子 PR 的堆叠与准入表明确每个审查单元的父 PR、能力前置和核心评审问题；另列相邻 PR 的接口冻结、变更后的下游回归范围和进一步拆分规则。本文维护能力范围与验证状态，清单不另行维护一套完成状态。
