...
执行时配合[逐 PR 交付清单](2026-09-10-android-pr-checklist.md)：该文按 28 个实际必需 PR 列出准入条件、代码落点、交接材料和阻塞条件，并展开 WASM、渲染选型、输入草稿、授权读取、可靠发送、Diff、文件、通知后台及发行的具体实施顺序；补充各阶段的连续验收流程、失败阻塞范围以及 PR-06 前需冻结的测试输入。另列相邻 PR 的接口冻结、变更后的下游回归范围和进一步拆分规则。本文维护能力范围与验证状态，清单不另行维护一套完成状态。
