...
执行时配合[逐 PR 交付清单](2026-09-10-android-pr-checklist.md)：该文按 28 个实际必需 PR 列出准入条件、代码落点、交接材料和阻塞条件，并单列 WASM、渲染选型与可靠发送的决策包；补充各阶段的连续验收流程、失败阻塞范围以及 PR-06 前需冻结的测试输入。本文维护能力范围与验证状态，清单不另行维护一套完成状态。
