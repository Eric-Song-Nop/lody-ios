...
### PNG receipt timing diagnostic

The sheet/light diagnostic `feedback-default-sheet-light-png-diagnostic` uses the same APK `553d7744…`, runner `cc3d635` with the PNG timing helper in the working tree, and unchanged v2 deadlines. It passes: screenshot starts 0.247740 seconds after input begins and completes at 1.630570 seconds; later absence is observed at 8.892786 seconds. All five PNGs and five-second samples spanning the 40.604867-second recording were reviewed. The Unicode toast is readable above the gesture area, later absent, and Back returns to Settings. The timeout preference restores to `null`.
