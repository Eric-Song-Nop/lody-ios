...
AXe 1.8.0 is now installed with the CI-pinned archive checksum verified. A fresh managed iOS Simulator run is retrying the licenses and home cases using the existing signed Debug app; its results remain pending until the runner finishes and artifacts are reviewed.
