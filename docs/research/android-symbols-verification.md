...
## Dark page and sheet results

The same `3382ae3e…` APK passes both dark hosts on API 36 ARM64 with `symbols-v1`. Page runner checkout was `0c3a71f` clean; sheet runner checkout was `91b23bd` clean. Each verifies all 39 targets at 48 dp, seven current-name activations, disabled final Next and actual Back removal. Each restores its original night setting (page: no; sheet: yes) and shuts down its owned emulator.

All eight screenshots per host and full-duration five-second recording samples are reviewed: page 94.023411 seconds, sheet 100.477856 seconds. White static and blue action glyphs are readable, matched and semantically distinct; long labels fit and native headers do not overlap. Page samples include the returned Projects host; sheet samples end on the final catalog, with dismissal established separately by the hierarchy assertion.

Evidence directories are `.artifacts/android/symbol-catalog-page-dark` and `.artifacts/android/symbol-catalog-sheet-dark`, each with the result, seven batch PNG/XML pairs, recording and visual review. Together with the light results, this completes the scoped symbol host/theme matrix. It does not close TalkBack or full PR-05b acceptance.

## Corresponding iOS build

After the symbol matrix and failed TalkBack attempt stopped their emulator, the managed `Android symbol catalog iOS regression` lease ran asset generation, a normal-signing Debug simulator build and `codesign --verify --deep --strict`. All exited successfully at checkout `1b03fe0` with verifier-only edits. The lease was released. Logs are `.artifacts/android/ios-regression/symbol-catalog-assets.log`, `symbol-catalog-build.log` and `symbol-catalog-lease.log`. No signing override was used. This proves the corresponding source/assets still build for iOS; it does not replace iOS UI or the separately failing parent CI automation.
